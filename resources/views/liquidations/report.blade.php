...
<body>
    <div class="header">
        ...
        <h1>CIERRE APLICADO DEL CUADRE DIARIO DE LISTADO DE RUTA
            {{ $report['seller'] ? strtoupper($report['seller']->city->name) : 'GENERAL' }}</h1>
        <h2>Cobrador Encargado de este Cierre: {{ $report['user'] ? $report['user']->name : 'TODOS' }}</h2>
        <h3>FECHA DEL CIERRE: {{ $report['report_date'] }}</h3>
    </div>
    ...
    <div class="footer">
        <table class="summary-table">
            <tr>
                ...
                <td class="text-right">{{ $report['total_credits'] ?? 0 }}</td>
            </tr>
            <tr>
                <td><strong>RECIBOS CON PAGOS:</strong></td>
                <td class="text-right">{{ $report['with_payment'] ?? 0 }}</td>
            </tr>
            <tr>
                <td><strong>RECIBOS SIN PAGOS:</strong></td>
                <td class="text-right">{{ $report['without_payment'] ?? 0 }}</td>
            </tr>
            <tr>
                <td><strong>TOTAL RECAUDADO:</strong></td>
                <td class="text-right">$ {{ number_format($report['total_collected'] ?? 0, 2) }}</td>
            </tr>
            ...
            @if (isset($report['total_incomes']))
                <tr>
                    ...
                    <td class="text-right">$ {{ number_format($report['total_incomes'], 2) }}</td>
                </tr>
            @endif
            ...
            @if (count($report['new_credits'] ?? []) > 0)
                <tr>
                    ...
                    <td class="text-right">{{ count($report['new_credits'] ?? []) }}</td>
                </tr>
            @endif
        </table>
