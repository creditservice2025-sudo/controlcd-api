...
class LiquidationService
{
    ...
    public function calculateLiquidationMetrics($sellerId, $date, $forcedInitialCash = null, $timezone = 'America/Lima')
    {
        ...
        if ($initialCash === null) {
            ...
            // Log if we are in recalculate context and initial_cash changed (optional, but head had it)
            if (isset($liquidation) && $liquidation->initial_cash != $initialCash) {
                \Log::info("Autocorrecting initial_cash for liquidation ID {$liquidation->id} during metrics calculation: {$liquidation->initial_cash} -> {$initialCash}");
            }
        }
