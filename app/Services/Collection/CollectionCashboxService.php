...
class CollectionCashboxService
{
    ...
    {
        $fecha = $this->openedOnDate($c, $primerMovimiento);

        return $fecha ? \Carbon\Carbon::parse($fecha)->format('d/m/Y') : null;
    }

    /**
     * La MISMA fecha que openedOn(), pero en formato máquina (YYYY-MM-DD).
     *
     * El frontend la necesita sin formatear para acotar el calendario y para
     * comparar rangos: con la versión dd/mm/aaaa esas comparaciones son texto y
     * ordenan mal. Antes se usaba `created_at` de la caja para eso, que es
     * justamente la fecha equivocada cuando el backfill creó la ficha después
     * de sus propios movimientos.
     */
    private function openedOnDate(CollectionCashbox $c, ?string $primerMovimiento = null): ?string
    {
    ...
    ): array
    {
        return [
            ...
            // La misma fecha sin formatear: el calendario y los rangos del
            // frontend comparan por texto y necesitan YYYY-MM-DD.
            'opened_on_date'  => $this->openedOnDate($c, $primerMovimiento),
