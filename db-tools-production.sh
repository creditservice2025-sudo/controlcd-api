...
BACKUP_DIR="$BASE_DIR/backups-prod"

# Configuración de conexión a PRODUCCIÓN (acceso remoto)
DB_HOST="${DB_HOST:-128.199.1.223}"
DB_PORT="${DB_PORT:-3306}"
DB_USER="${DB_USER:-controlcd_admin}"
DB_NAME_DEFAULT="${DB_NAME_DEFAULT:-controlcd_prod}"
DB_PASS="${DB_PASS:-55a30c6c-2473-4279-823a-261f2cce92ee}"
# ========================================================
...
prompt_password() {
  # Si DB_PASS ya está configurada, usarla; si no, pedirla
  if [[ -z "${DB_PASS:-}" ]]; then
...
restore_db() {
  ...
  warn "⚠️  ADVERTENCIA: Estás a punto de restaurar en PRODUCCIÓN"
  warn "   Servidor: $DB_HOST"
  warn "   Base de datos: $db"
  echo
  read -r -p "¿Estás SEGURO de continuar? (escribe 'SI' en mayúsculas): " confirm
  if [[ "$confirm" != "SI" ]]; then
  ...
  info "Restaurando en BD de PRODUCCIÓN: $db"
  ...
  ok "✓ Restore completado en BD de PRODUCCIÓN: $db"
}

next_increment_name() {
  local base_name="$1"   # p.ej: controlcd_prod_2025-11-15
  local ext="$2"         # .sql.gz
  local dir="$3"
...
backup_db() {
  ...
  info "Generando backup de BD de PRODUCCIÓN: $db -> $dest"
  mysqldump -h "$DB_HOST" -P "$DB_PORT" -u "$DB_USER" -p"$DB_PASS" \
    --single-transaction --quick --lock-tables=false \
    --default-character-set=utf8mb4 \
    "$db" | gzip -9 > "$dest"

  ok "✓ Backup de PRODUCCIÓN creado: $dest"
  ls -lh "$dest"
}
...
print_config() {
  cat <<EOF
╔════════════════════════════════════════════════════╗
║  🔴 HERRAMIENTAS DB - PRODUCCIÓN 🔴               ║
╚════════════════════════════════════════════════════╝

Conexión actual:
  Host: $DB_HOST (PRODUCCIÓN - acceso remoto)
  Port: $DB_PORT
  User: $DB_USER (usuario remoto)
...
⚠️  IMPORTANTE: Este script conecta al servidor de PRODUCCIÓN
   Ten mucho cuidado con las operaciones de restore.

✅ Configurado para acceso remoto desde tu PC

Puedes sobreescribir con variables de entorno:
  DB_HOST, DB_PORT, DB_USER, DB_NAME_DEFAULT, DB_PASS
EOF
}
...
# Ejemplos:
#   ./db-tools-production.sh --restore --db controlcd_prod --file /path/file.sql.gz
#   ./db-tools-production.sh --backup --db controlcd_prod
if (( $# > 0 )); then
  ACTION=""
  FILE=""
  DB="$DB_NAME_DEFAULT"
