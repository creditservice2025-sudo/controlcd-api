...
NGINX_CONF="server-conf/nginx-production-versions.conf"

echo "🚀 Configurando servidor de versiones (PRODUCCIÓN)..."

# 1. Crear directorios
echo "📂 Creando directorios en el servidor..."
ssh -i $SSH_KEY $SERVER_USER@$SERVER_IP "mkdir -p /var/www/control-cd-versions"
...
ssh -i $SSH_KEY $SERVER_USER@$SERVER_IP "certbot --nginx -d apk.control-cd.com --non-interactive --agree-tos -m admin@control-cd.com --redirect"

echo "✅ Servidor de Producción configurado exitosamente!"
